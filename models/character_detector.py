...
class Character_Detector():
    ...
    def split_character(self, image: np.ndarray, canvas_size: int = 1280,
        ...
        # border: np.ndarray = np.ones((5, image.shape[1], image.shape[2]))  # Visualization for test.
        # image_vis_1: np.ndarray = image.copy()  # Visualization for test.
        # image_vis_2: np.ndarray = cv2.cvtColor(  # Visualization for test.
        #     heatmap.copy() * 255,
        #     cv2.COLOR_GRAY2RGB,
        # ).astype(np.uint8)
        # image_vis_3: np.ndarray = cv2.cvtColor(  # Visualization for test.
        #     cv2.applyColorMap(
        #         (np.clip(heatmap.copy(), 0, 1) * 255).astype(np.uint8),
        #         cv2.COLORMAP_JET,
        #     ),
        #     cv2.COLOR_BGR2RGB,
        # )
        # image_vis_4: np.ndarray = cv2.addWeighted(  # Visualization for test.
        #     image_vis_1.copy(),
        #     0.5,
        #     image_vis_3.copy(),
        #     0.5,
        #     0,
        # )
        # image_vis: np.ndarray = cv2.vconcat(  # Visualization for test.
        #     [
        #         cv2.cvtColor(image_vis_1, cv2.COLOR_RGB2BGR),
        #         cv2.cvtColor(image_vis_2, cv2.COLOR_RGB2BGR),
        #         cv2.cvtColor(image_vis_3, cv2.COLOR_RGB2BGR),
        #         cv2.cvtColor(image_vis_4, cv2.COLOR_RGB2BGR),
        #     ],
        # )
        # imshow(  # Visualization for test.
        #     image_vis,
